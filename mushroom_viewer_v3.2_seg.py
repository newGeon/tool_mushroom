...
class MushroomMain(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowTitle("[Segmentation] 버섯 라벨링 저작도구 Ver1.0")
        ...
        self.label_title = QtWidgets.QLabel("[Segmentation] 버섯 라벨링 저작도구")
        ...
        self.image_label.mousePressEvent = self.getPixcel
    ...
    def hangul_file_path_image_read(self, file_name):
        file_path = os.path.join(self.open_path, file_name)
    ...
    def open_folder(self):
        self.json_data = []
        self.open_path = str(QFileDialog.getExistingDirectory(self, "이미지 파일이 있는곳을 지정해주세요.", "./tool_mushroom/data"))
    ...
    def set_data(self, index):        
        ...
        self.lbl_json_file_name_value.setText(image_file_name)
        self.lbl_json_index_value.setText(str(index + 1) + " / " + str(self.total_count))
        
        self.current_index = index

        c_image = self.hangul_file_path_image_read(image_file_name)
        ...
        for idx, annotation_data in enumerate(data_info['ANNOTATION_INFO']):
            ...
            segmentation_list = annotation_data['SEGMENTATION']
            
            if segmentation_list is not None:
                i = 2
                mushroomType = 1

                first_pt = [int(segmentation_list[0]), int(segmentation_list[1])]
                ...
                while i < len(segmentation_list):
                    nxt_pt = [int(segmentation_list[i]), int(segmentation_list[i+1])]
                    if(nxt_pt[0] == -1) : nxt_pt[0] = 0
                    c_image = cv2.line(c_image, (prv_pt[0], prv_pt[1]), (nxt_pt[0], nxt_pt[1]), self.COLOR[mushroomType-1], 2)
                    prv_pt = nxt_pt
                    i += 2
                
                c_image = cv2.line(c_image, (prv_pt[0], prv_pt[1]), (first_pt[0], first_pt[1]), self.COLOR[mushroomType-1], 2)
        ...
        p_ixmap = p_ixmap.scaled(p_ixmap.width() / 2, p_ixmap.height() / 2)
    ...
    def getPixcel(self, event):        
        if event.button() == QtCore.Qt.LeftButton and self.select_category >= 0 and self.job_type == 'Segmentation':
            # Segmentation 용
            x = event.pos().x()
            y = event.pos().y()

            self.flag = False
            self.new_segmentaion_list.append(event.pos().x() * 2)
            self.new_segmentaion_list.append(event.pos().y() * 2)

            self.draw_segmentaion()

        if event.button() == QtCore.Qt.RightButton and self.select_category >= 0 and self.job_type == 'Segmentation':            
            self.flag = False
            self.new_segmentaion_list = []

            self.set_data(self.current_index)
                

    def draw_segmentaion(self):
        if self.new_segmentaion_list is not None:
            
            len_seg = len(self.new_segmentaion_list)

            c_h, c_w, c_c = self.current_image.shape

            img_pil = Image.fromarray(cv2.cvtColor(self.current_image, cv2.COLOR_BGR2RGB))
            c_image = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)        
            c_image = cv2.circle(c_image, (self.new_segmentaion_list[0], self.new_segmentaion_list[1]), 3, (255,255,255), -1)

            i = 2
            while i < len_seg:
                c_image = cv2.circle(c_image, (self.new_segmentaion_list[i], self.new_segmentaion_list[i+1]), 3, (255,255,255), -1)
                c_image = cv2.line(c_image, (self.new_segmentaion_list[i-2], self.new_segmentaion_list[i-1]), (self.new_segmentaion_list[i], self.new_segmentaion_list[i+1]), self.COLOR[0], 2)
                i += 2
            ...
            if len_seg > 2:
                ...
                distance = math.sqrt(math.pow(self.new_segmentaion_list[0] - self.new_segmentaion_list[len_seg-2], 2) + math.pow(self.new_segmentaion_list[1] - self.new_segmentaion_list[len_seg-1], 2))
                
                if distance < 5.0:
                    # 두 점 사이가 5.0 미만 이면 완성 (끝 점 정확하게 클릭에 어려워서 편의 기능 추가)
                    self.new_segmentaion_list[len_seg-2] = self.new_segmentaion_list[0]
                    self.new_segmentaion_list[len_seg-1] = self.new_segmentaion_list[1]

                if (self.new_segmentaion_list[0] == self.new_segmentaion_list[len_seg-2]) and (self.new_segmentaion_list[1] == self.new_segmentaion_list[len_seg-1]):
                    # segmentaion 점을 이용한 BBox 그리기
                    x_list = self.new_segmentaion_list[0::2]
                    y_list = self.new_segmentaion_list[1::2]
                    ...
                    new_bbox = {}
                    new_bbox['ID'] = 1
                    new_bbox['BOUNDING_BOX_X_COORDINATE'] = x_min
                    new_bbox['BOUNDING_BOX_Y_COORDINATE'] = y_min
                    new_bbox['BOUNDING_BOX_WIDTH'] = x_max - x_min
                    new_bbox['BOUNDING_BOX_HEIGHT'] = y_max - y_min
                    new_bbox['SEGMENTATION'] = self.new_segmentaion_list
                    ...
                    self.categories_pressed()
                    self.categories_toggled()
...
if __name__ == '__main__':
    # 이미지 파일만 Read 해서 Segmentaion 그리는 용도 (단순 그리기, 저장 기능 없음)
