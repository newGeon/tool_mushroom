...
class MushroomMain(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowTitle("[BBox] 버섯 라벨링 저작도구 Ver1.0")
        ...
        self.label_title = QtWidgets.QLabel("[BBox] 버섯 라벨링 저작도구")
    ...
    def getDragMouse(self, event):
        if self.select_category >= 0 and self.flag and self.job_type == 'BBox':
    ...
    def getStopMouse(self, envet):
        if self.select_category >= 0 and self.flag and self.job_type == 'BBox':
            self.draw_bbox()

            new_bbox = {}
            new_bbox['ID'] = 1
            new_bbox['BOUNDING_BOX_X_COORDINATE'] = self.begin.x() * 2
            new_bbox['BOUNDING_BOX_Y_COORDINATE'] = self.begin.y() * 2
            new_bbox['BOUNDING_BOX_WIDTH'] = (self.end.x() - self.begin.x()) * 2
            new_bbox['BOUNDING_BOX_HEIGHT'] = (self.end.y() - self.begin.y()) * 2
    ...
    def draw_bbox(self):        
        ...
        c_image = cv2.rectangle(self.current_image, (x1, y1), (x2, y2), (255,255,255), 5)
        img_pil = Image.fromarray(cv2.cvtColor(c_image, cv2.COLOR_BGR2RGB))
        img_draw = ImageDraw.Draw(img_pil)
        img_draw.text((x1, y1+5), 'new 버섯', font=ImageFont.truetype('./fonts/gulim.ttc', 45), stroke_width=1, fill=(0, 255, 255))
        c_image = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)
